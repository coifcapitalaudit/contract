...
contract CommunityInvestmentFundContract is ERC20, Ownable {
    ...
    function updateBuyFees(
        ...
        require((_newBuyLiquidityFee.add(_newBuyMarketingFee).add(_newBuyPool1Fee).add(_newBuyPool2Fee).add(_newBuyPool3Fee))<= MAX_FEE.mul(FEE_FACTOR), "Error: new desired fee over max limit");
    ...
    function updateSellFees(
        ...
        require((_newSellLiquidityFee.add(_newSellMarketingFee).add(_newSellPool1Fee).add(_newSellPool2Fee).add(_newSellPool3Fee))<= MAX_FEE.mul(FEE_FACTOR), "Error: new desired fee over max limit");
    ...
    function updateTxFees(
        ...
        require((_newTxLiquidityFee.add(_newTxMarketingFee).add(_newTxPool1Fee).add(_newTxPool2Fee).add(_newTxPool3Fee))<= MAX_FEE.mul(FEE_FACTOR), "Error: new desired fee over max limit");
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if (!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]) {
                liquidityFee = amount.mul(txLiquidityFee).div(FEE_FACTOR_PERCENT);
                marketingFee = amount.mul(txMarketingFee).div(FEE_FACTOR_PERCENT);
                pool1Fee = amount.mul(txPool1Fee).div(FEE_FACTOR_PERCENT);
                pool2Fee = amount.mul(txPool2Fee).div(FEE_FACTOR_PERCENT);
                pool3Fee = amount.mul(txPool3Fee).div(FEE_FACTOR_PERCENT);
            } else {
                bool isSell = automatedMarketMakerPairs[to] ? true : false;

                liquidityFee = isSell ? amount.mul(sellLiquidityFee).div(FEE_FACTOR_PERCENT) : amount.mul(buyLiquidityFee).div(FEE_FACTOR_PERCENT);
                marketingFee = isSell ? amount.mul(sellMarketingFee).div(FEE_FACTOR_PERCENT) : amount.mul(buyMarketingFee).div(FEE_FACTOR_PERCENT);
                pool1Fee = isSell ? amount.mul(sellPool1Fee).div(FEE_FACTOR_PERCENT) : amount.mul(buyPool1Fee).div(FEE_FACTOR_PERCENT);
                pool2Fee = isSell ? amount.mul(sellPool2Fee).div(FEE_FACTOR_PERCENT) : amount.mul(buyPool2Fee).div(FEE_FACTOR_PERCENT);
                pool3Fee = isSell ? amount.mul(sellPool3Fee).div(FEE_FACTOR_PERCENT) : amount.mul(buyPool3Fee).div(FEE_FACTOR_PERCENT);
            }
...
contract DividendDistributor is IDividendDistributor, Ownable {
                ...
                if (shareholderIndexes[_shareholder] < _payoutPool1ShareholderCount) {
                    if (currentIndexPool1 < shareholderIndexes[_shareholder]) {
                        if(shares[_shareholder].amount < _amountNew){
                            shares[_shareholder].amountExcludedBuyPool1 = (_amountNew.sub(shares[_shareholder].amount)).add(shares[_shareholder].amountExcludedBuyPool1);
                        }
                    }
                }
            }
            ...
            if (_processPool2Active){
                if (shareholderIndexes[_shareholder] < _payoutPool2ShareholderCount) {
                    if (currentIndexPool2 < shareholderIndexes[_shareholder]) {
                        if(shares[_shareholder].amount < _amountNew) {
                            shares[_shareholder].amountExcludedBuyPool2 = (_amountNew.sub(shares[_shareholder].amount)).add(shares[_shareholder].amountExcludedBuyPool2);
                        }
                    }
                }
            }
    ...
    function payoutDividendsPool1(
        ...
        if(shares[_shareholder].amount <= shares[_shareholder].amountExcludedBuyPool1){
            ...
            amount = ((shares[_shareholder].amount).sub(shares[_shareholder].amountExcludedBuyPool1)).mul(_payoutPool1DividendsPerShare).div(dividendsPerShareAccuracyFactor);
    ...
    function payoutDividendsPool2(
        ...
        if(shares[_shareholder].amount <= shares[_shareholder].amountExcludedBuyPool2){
            ...
            amount = ((shares[_shareholder].amount).sub(shares[_shareholder].amountExcludedBuyPool2)).mul(_payoutPool2DividendsPerShare).div(dividendsPerShareAccuracyFactor);
