...
contract MemecoifContract is ERC20, Ownable {
    ERC20 public constant WBNB = ERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); /* main chain */

    uint256 public constant MAX_FEE = 10; //total max fee = 10%
    ...
    // initial setup for fair stealth launch: 
    // a single wallet can hold up to 1% of the total supply
    // max transaction: up to 1% of the total supply
    uint256 public totalSupplyInit = 100_000_000 * (10**_decimals);
    uint256 public maxWalletAmount = 1_000_000 * (10**_decimals);
    uint256 public maxTransactionAmount = maxWalletAmount;
    ...
    address public idoWallet1;
    address public idoWallet2;
    ...
    bool public isLimitsRemoved = false; //init value
    ...
    event IdoWallet1Updated(address indexed newIdoWallet1, address indexed oldIdoWallet1);
    event IdoWallet2Updated(address indexed newIdoWallet2, address indexed oldIdoWallet2);
    ...
    event LimitsDeactivated(bool newStatus);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiqudity);

    constructor() ERC20("MEMECOIF", "MEMECOIF"){
    ...
    function setIdoWallet1(address _newIdoWallet1) public onlyOwner validateAddressNotZero(_newIdoWallet1) {
        require(_newIdoWallet1 != idoWallet1, "Error: The idoWallet1 is already this address");
        excludeFromFees(_newIdoWallet1, true);
        emit IdoWallet1Updated(_newIdoWallet1, idoWallet1);
        idoWallet1 = _newIdoWallet1;
    }

    function setIdoWallet2(address _newIdoWallet2) public onlyOwner validateAddressNotZero(_newIdoWallet2) {
        require(_newIdoWallet2 != idoWallet2, "Error: The idoWallet2 is already this address");
        excludeFromFees(_newIdoWallet2, true);
        emit IdoWallet2Updated(_newIdoWallet2, idoWallet2);
        idoWallet2 = _newIdoWallet2;
    }
    ...
    function _transfer(
        ...
        if(!isLimitsRemoved){ //limits cannot be reactivated once removed
            if (
                from != owner() && to != owner() &&
                from != idoWallet1 && to != idoWallet1 &&
                from != idoWallet2 && to != idoWallet2 &&
                to != address(0xdead) && to != marketingWallet
            ) {
                // SELL - LP pair is excluded from maxWalletAmount
                if (automatedMarketMakerPairs[to]) {
                    require(amount <= maxTransactionAmount, "Error: Sell amount exceeds the maxTransactionAmount.");
                }
                // BUY or simple Transfer - max transaction limit and max wallet limit
                else {
                    require(amount <= maxTransactionAmount, "Error: Buy or Transfer amount exceeds the maxTransactionAmount.");
                    require((amount + balanceOf(to)) <= maxWalletAmount, "Error: Max wallet amount exceeded");
                }
            }
        }
    ...
    //limits cannot be reactivated once removed
    function removeLimits() external onlyOwner {
        maxTransactionAmount = totalSupplyInit;
        maxWalletAmount = totalSupplyInit;
        isLimitsRemoved = true; 
        emit LimitsDeactivated(isLimitsRemoved);
    }
