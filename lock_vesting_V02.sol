...
contract TokenVesting is Ownable {
  ...
  function release(IERC20 _token) public onlyOwner {
    ...
    uint256 tokenBalanceContract = _token.balanceOf(address(this));
    require(tokenBalanceContract > 0, "Error: no tokens in contract to release");

    if(tokenBalanceContract >= unreleased){
        ...
        released[address(_token)] = released[address(_token)].add(tokenBalanceContract);
        _token.safeTransfer(beneficiary_, tokenBalanceContract);
        emit TokensReleased(_token, tokenBalanceContract);
    }
  }
